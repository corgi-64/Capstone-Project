...
const MovieDetailsPage = () => {
    ...
    return (
        ...
        <div style={{ color: "white" }}>
            <h1 style={{color: "#905ca0", textShadow: "2px 2px 5px rgba(0, 0, 0, 0.5)"}}>{movie.title}</h1>
            <img
            ...
            />
            <ButtonGroup sx={{ mt: 2 }} >
            <Button variant="contained" style={{background:"#5e3d7b"}} onClick={handleWantToWatch}>Want To Watch </Button> 
            ...
            <p><strong style={{color: "#89CFF0"}}>Rating:</strong> {movie.vote_average} / 10</p>
            <p><strong style={{color: "#89CFF0"}}>Genres:</strong> {movie.genre_ids ? movie.genre_ids.map((genreId) => genres.find((genre) => genre.id === genreId)?.name).join(", ") : "No genres available"}</p>
            <p><strong style={{color: "#89CFF0"}}>Popularity:</strong> {movie.popularity}</p>
            <p><strong style={{color: "#89CFF0"}}>Release:</strong> {movie.release_date}</p>
            <p><strong style={{color: "#89CFF0"}}>Runtime:</strong> {movie.runtime !== "N/A" ? `${Math.floor(movie.runtime / 60)}h ${movie.runtime % 60}m` : "N/A"}</p>
            <p>{movie.overview}</p>
            <p>
                <strong style={{ color: "#89CFF0" }}>Credits:</strong>{" "}
